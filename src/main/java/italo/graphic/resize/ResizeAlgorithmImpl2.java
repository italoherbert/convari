...
public class ResizeAlgorithmImpl2 extends AbstractResizeAlgorithm {

	private int radial = 2;
	
	public ResizeAlgorithmImpl2() {}
	
	public ResizeAlgorithmImpl2(int radial) {
		this.radial = radial;
	}
		...
		for( int i = 0; i < h2; i++ ) {
			for( int j = 0; j < w2; j++ ) {
				...
				for( int k = -radial; k <= radial; k++ ) {
					for( int l = -radial; l <= radial; l++ ) {
						int x = Math.round( j * f2 ) + l;
